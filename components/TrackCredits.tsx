...
export default function TrackCredits({ tracks }: TrackCreditsProps) {
  if (!tracks.length) return null;

  return (
    <section className="mt-20 border-t border-white/15 pt-8 sm:mt-28 sm:pt-10" aria-labelledby="track-credits-heading">
      <div className="mb-4 flex items-baseline justify-between gap-6 sm:mb-6">
        <h2 id="track-credits-heading" className="text-xs tracking-[0.2em] font-gotham-bold sm:text-sm">
          TRACK CREDITS
        </h2>
        <p className="text-[9px] tracking-[0.16em] opacity-40 sm:text-xs">{String(tracks.length).padStart(2, "0")} TRACKS</p>
      </div>
