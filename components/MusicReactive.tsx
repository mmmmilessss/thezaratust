...
export default function MusicReactive({ envelope, children, className = "" }: { envelope?: string; children: React.ReactNode; className?: string }) {
  ...
  useEffect(() => {
    ...
    const tick = () => { if (!state || !ref.current) return; const position = state.positionMs + (state.isPlaying ? performance.now() - state.updatedAt : 0); const target = state.isPlaying && !state.isBuffering ? values[Math.min(values.length - 1, Math.floor(position / 25))] ?? 0 : 0; level += (target - level) * (target > level ? 0.22 : 0.08); ref.current.style.opacity = String(1 - level * 0.22); if (state.isPlaying || level > .005) raf = requestAnimationFrame(tick); };
